...
export default function LoginPage() {
  ...
  const getRedirectUrl = () => {
    ...
    return `${url}/${locale}/auth/callback`;
  };
  ...
  useEffect(() => {
    ...
    if (error === 'auth_failed') {
      addToast({ message: t('auth.login_failed'), type: 'error' });
    }
      ...
      if (event === 'SIGNED_IN' && session && !isProcessingRef.current) {
        ...
        timeoutId = setTimeout(async () => {
          try {
            ...
            if (currentSession) {
              addToast({ message: t('auth.login_success'), type: 'success' });
              // Use router.push with forced refresh
              router.push(`/${locale}`);
              router.refresh();
            } else {
